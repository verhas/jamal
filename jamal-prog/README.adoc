...
The macro `pi` returns the number itself in a string format.
The other macros take the arguments and perform the operation on the number with the provided arguments.
The result is always the string representation of the result, and it also changes the value of the number.
...
You can call these macros as real macros without using the BASIC interpreter, but originally they are meant to be used as functions in the BASIC language.