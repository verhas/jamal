...
The macro `yaml:set` can define a user defined macro from an already existing yaml macro.
It is similar to `yaml:define` but this macro does not parse a text and interpret it as yaml formatted text.
Instead, it uses an already defined yaml user defined macro and uses some part of it and assignes that to a user defined macro name.
