...
When the containing object is a map then the `ATTR` object can be a simple string, or it can be a map.
When the `ATTR` is a simple string then the key of it will be used as attribute name.
...
Any furter keys will result error.
