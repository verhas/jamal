...
In the next chapter we will look at an example how we shoudl do that.

==== Splitting the Input

If you look at the core built-in macro `if` then you can see that it does not have a special syntax.
It just has three parameters and in case the first parameter is true, then it returns the second parameter, otherwise the third if that exists.
...
Looking at the syntax we should interpret the separator a bit abstract.
It can be literally the `/` character, but it can be something else as well.
