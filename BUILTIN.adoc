...
The simplest way is to do that is using the classes from the `jamal-testsupport` library.
This library contains test utilities that mimic a Jamal environment for the macro and also provide useful test mocks.
...
There is a use to still define a parameter as a macro though.
