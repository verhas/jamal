...
NOTE: There is one exception, when you have to use `()` even for an empty parameter list.
...
They have to be registered by a name in the Jama engine, hence they need to implement the `Identified` interface, and they have to evaluate to a string, hence they need to implement the `Evaluable` interface.
...
For example, the macro `counter:define` from the Snippet package creates a "user-defined" macro that results a number when used, but it also changes the value at every invocation.

Starting with the release 2.3.0 the macro supports the option `class`.
This option can provide the class name that implements the macro.
The class has to implement the `Identified` and `Evaluable` interfaces.
It optionally can implement the `Configurable` interface.
...
This can be prevented using the link:verbatim.adoc[`verbatim`] macro.
You can also read more details on the macro evaluation order in link:evaluation_order.adoc[`evaluation:order`].
...
If, for any reason, you need to evaluate the result of such a macro you can use the macro link:eval.adoc[`eval`] or `!` when using th e macro.
...
For example, modifying our example from the link:begin.adoc[`begin`] section:
...
If you use a user-defined macro that is undefined, it will result in an error.
...
* it was explicitly undefined using the link:undefine.adoc[`undefine`] macro.
...
If the first argument of the macro is either `$macro` or `$_`, then this parameter will hold the macro name used.
...
The number of parameters should be exactly the same as the number of arguments unless the `{@options :lenient}` was specified, or the `...` was used to denote optional arguments.
...
* `fail`, `noRedefine`, `noRedef`, `failIfDefined` says that the macro should not be redefined.
You can use this option instead of the `!` character.

* `tail` says that the macro should be parsed as a tail macro.

* `global` says that the macro should be defined in the global scope.

* `export` says that the macro should be exported.
...
The modifying characters remain part of the language for backward compatibility only.
...
In this case, the parameters must be named in arbitrary order just like the options for the built-in macros.
