...
You can also read more details on the macro evaluation order in link:evaluation:order[`evaluation:order`].
...
If, for any reason, you need to evaluate the result of such a macro you can use the macro link:eval[`eval`] or `!` when using th e macro.
...
For example, modifying our example from the link:begin[`begin`] section:
