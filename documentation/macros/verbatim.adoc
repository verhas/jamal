...
The macro `c` is defined using the `#` character, therefore, Jamal will process the body of the macro before processing the built-in macro link:define[`define`] itself.
...
If the macro is user defined, then it evaluates as described in the document link:define[`define`].
...
* skipped using the macro link:verbatim[`verbatim`] if the macro is user defined, or
* enforced using a `!` in front of the `@` or `#` character if the macro is built-in.

The use of the `!` character in front of a built-in macro is similar to the use of the macro link:eval[`eval`].
For example
...
The macro link:eval[`eval`] deletes this new line, while using the `!` in front of the macro does not.
...
Note that the macro link:verbatim[`verbatim`] is a special one because it is hardwired into the evaluation logic of Jamal and it is not a "real" built-in macro.
...
Using the macro link:options[`options`] as `{@options omasalgotm}` you can switch to the old algorithm.
