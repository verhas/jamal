...
1. <<Starting,Starting Jamal>>

2. <<SimpleExample,Simple Example>>

3. <<#OtherMacros,Other Macros>>

    i)     <<comment,`comment`>>
    ii)    <<block,`block`>>
    iii)   <<begin,`begin` and `end`>>
    iv)    <<define,`define`>>
    v)     <<eval,`eval`>>
    vi)    <<env,`env`>>
    vii)   <<export,`export`>>
    ...
    xiv)   <<if,`if`>>
    xv)    <<ident,`ident`>>
    xvi)   <<verbatim,`verbatim`>>
    xvii)  <<options,`options`>>
    xviii) <<try,`try`>>
    xix)   <<JamalENV,Jamal Environment Variables>>
    xx)    <<JamalAPI,Jamal API>>
...
The same way the text between the `{@begin}` and `{@end}` will be evaluated.
...
Using the `()` after the identifier of the macro is optional and the result is exactly the same as if it omitted.
...
[source]
----
Macro 'undefinedMacro' is not defined.
...
and the output is
{@comment the 'sample' macro evaluation does not make it possible to demonstrate this here
would be too complex. It is demonstrated in the javax0/jamal/extensions/TestUserDefinedPostEval.java
}
[source]
----
{c}
...
False=False          <- separator character is 'f'
                                          condition is 'alse ', non-empty string
...
If the syntax does not match and of the previous cases, Jamal will use the syntax that is defined with the followinf "regular expression" like line:
