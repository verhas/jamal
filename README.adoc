...
The class is a utility to parse the first line, some particular part of the whole input of the built-in macro looking for parameters.
...
For example the `trimLines` macro in the snippet package can trim also vertically and the left trimming can go all to the left but it may also leave some margin.
...
This macro reserves the first line of the input of the macro and trims and returns only the rest of the lines.
The first line can contain paramaters.
...
In the cae of this macro the whole input till the macro closing character is parsed for parameters.
...
* parsing follows the same rules, users do not need to learn different syntaxes.
...
The macro first passes the input to the parsing method and later uses the resulting object to query the parameter values.
When a parameter is not defined there then the query algorithm tries to look up a user defined macro that has the name as the parameter.
If you set `{@define margin=2}` somewhere in your Jamal source, this value will be used by every `trimLine` following it unless the `margin` parameter is defined on the fist line of the input.
If the value is not defined as a parameter and it is also not defined as a user defined macro then the macros use default values.
...
The values can be represented enclosed between `"`.
This is a typical string representation.
...
It has to be represented as `pattern="Peter Verhas"`, because it contains a space.
...
Some macros use the parsing to parse for parameters only the first line.
...
The parsing stops only at the first new-line character, which is not part of any value.

If there are many parameters and the first line becomes too long, then the `\` character at the end of the line make the parser skip the new line and gon on parsing on the next line.
...
If there is any parameter, which the macro does not handle an error will occurr.

When the macro queries a parameter it uses type specific method.
If the parameter cannot be converted to the type an error will occur.

* Numeric parameters should be integer.
...
If there are only one value defined but the macro needs a list it will get a one element list.
You can specify only one value for such parameter using user defined macros.

The globally, or locally defined user macro will not be added to the list of parameters.
