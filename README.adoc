...
[source,bash]
----
java -cp $HOME/.m2/repository/com/javax0/jamal/jamal-engine/1.10.0-SNAPSHOT/jamal-engine-1.10.0-SNAPSHOT.jar:$HOME/.m2/repository/com/javax0/jamal/jamal-api/1.10.0-SNAPSHOT/jamal-api-1.10.0-SNAPSHOT.jar:$HOME/.m2/repository/com/javax0/jamal/jamal-tools/1.10.0-SNAPSHOT/jamal-tools-1.10.0-SNAPSHOT.jar:$HOME/.m2/repository/com/javax0/jamal/jamal-core/1.10.0-SNAPSHOT/jamal-core-1.10.0-SNAPSHOT.jar:$HOME/.m2/repository/com/javax0/jamal/jamal-cmd/1.10.0-SNAPSHOT/jamal-cmd-1.10.0-SNAPSHOT.jar: javax0.jamal.cmd.JamalMain options
----
...
This way the macro `defer` is similar to the macro `eval`.
====
...
- multiple `defer` macros are executed in the order as they were evaluated in the input during the Jamal processin.
...
[source]
----
The current version 1.10-SNAPSHOT is older than the required version. It has to be newer.
----
...
To be consistent the macros `include`, `eval`, and `defer` also use the `[` and the `]` characters.
...
For example, for the environment variable `JAMAL_CHECK_STATE`, the system property is `jamal.check.state`.
...
* `JAMAL_DEV_PATH` can define file replacements.
It may happen that you want to develop a macro package.
The package will be deployed to a repository and will have a file name of the form `https://myServer.com/myfile.jim`.
While you develop the package you want to test it locally and you do not want to deploy it to the server for every test run.
Still you want to use the final URL to reference the file.
Using this environment variable you can list the files that should be replaced.
Each entry in the variable has the format `file name=replacement file name`.
No spaces are allowed or stripped off.
The entries are separated using the <<argsplit,Standard Built-In Macro Argument Splitting>>.
Only absolute file names can be replaced.
Do not forget to `unset` the variable after use to avoid surprises.
