...
[source,bash]
----
java -cp $HOME/.m2/repository/com/javax0/jamal/jamal-engine/1.7.3-SNAPSHOT/jamal-engine-1.7.3-SNAPSHOT.jar:$HOME/.m2/repository/com/javax0/jamal/jamal-api/1.7.3-SNAPSHOT/jamal-api-1.7.3-SNAPSHOT.jar:$HOME/.m2/repository/com/javax0/jamal/jamal-tools/1.7.3-SNAPSHOT/jamal-tools-1.7.3-SNAPSHOT.jar:$HOME/.m2/repository/com/javax0/jamal/jamal-core/1.7.3-SNAPSHOT/jamal-core-1.7.3-SNAPSHOT.jar:$HOME/.m2/repository/com/javax0/jamal/jamal-cmd/1.7.3-SNAPSHOT/jamal-cmd-1.7.3-SNAPSHOT.jar: javax0.jamal.cmd.JamalMain options
----

It is not a user-friendly approach.
You do not want to type all the paths and the JARs every time you want to start Jamal.
For this reason, there is a `jamal.sh` that has the following content:
...
[source,text]
----
Usage: jamal option1=value1 ... optionN=valueN
open                is the macro opening string, defaults to '{'
close               is the macro closing string, defaults to '}'
pattern             include files that match this pattern (regex)
exclude             exclude files that match this pattern (regex)
source              the source directory, defaults to '.'
target              where the generated files are to put, defaults to '.'
directory           specifies the source and the target at the same time
depth               recursion depth into directories. Default is infinite.
from                file name transformation pattern (regex), defaults to '\.jam$'
to                  file name transformation pattern (string), defaults to ''
                        target_name = source_name.replaceAll(transformFrom,transformTo)
                        defaults to chopping off '.jam' extension
----

The command line is built up from the command and key-value pairs.
For example, to convert the single `pom.xml.jam` to `pom.xml`, you can issue the command

[source,bash]
----
./jamal.sh depth=1 pattern=pom.xml.jam
----

It will tell Jamal to process all the files that

* match the regular expression `pom.xml.jam`,

* search for such files in the current directory and below directories

* but not more than one level depth in the directories (essentially only in the current directory).

The argument pattern is interpreted as a regular expression.
That way, this use would also match, for example, `pomaxmlojam`.
...
Note that in some cases, the regular expression characters may be interpreted by the command line bash.
In that case, you have to use `\` in front of the character.

The options you can use with the command-line version of Jamal:

* `open` can define the macro opening string.
The default value is `{`.
The Jamal source file can later change this value any time using the <<sep,`sep`>> macro.

* `close` can define the macro closing string.
The default value is `}`.
The Jamal source file can later change this value any time using the <<sep,`sep`>> macro.

* `pattern` can define a regular expression that needs to match a file's name to process.
Jamal's command-line version scans the directories recursively, listing all the files.
It processes only the files where the name matches the pattern.

* `exclude` can define a regular expression that must not match a file's name to process.
It is the opposite of the option `pattern`.

* `source` can define the source directory where the search for processable files starts.
The default directory is '.'

* `target` can define the target directory where Jamal will generate the output files.
The default directory is '.'.
When a source file is in a subdirectory under the `source` directory, then the resulting file will also be created in a subdirectory mirroring the source structure.

* `directory` is a shorthand option specifying the same value for `source` and `target`. (1.7.3 and later)

* `depth` can define the levels of recursion in the directories looking for source files.
The default value is practically infinite.
(Actual value is `Integer.MAX_VALUE`.) (1.7.3 and later)

* `from` can specify a regular expression to be used to convert the file name.
When the output file name is calculated, the input file name is converted, calling the Java `String` method `replaceAll`.
It replaces any substring matching the regular expression as the first argument to the second argument's string.
The default value is `\.jam$`. See also the option `to`.

* `to` can specify the replacement string in the call to `replaceAll`, transforming the input file name to the output file name.
The default value is `''` empty string. If neither `from` nor `to` is set then this will call `replaceAll("\.jam$",""")` on the input file name.
Since `\.jam$` matches the extension `.jam` at the end of the file name, this will be replaced by an empty string.
That way, for example, `pom.xml.jam` will be converted to `pom.xml`.
Feel free to define other file name transformations and feel free to use regular expression capturing groups and `$i` placeholders.
...
This startup also loads all the Jamal extensions, including `snippet`, `scriptbasic`, `groovy`, `ruby`, `plantuml`, and `debug` (1.7.3 and later).
