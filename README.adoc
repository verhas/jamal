...
* use built-in macros (there is a purposefully limited number of them in the core module and there are a lot in different other modules),
...
The Plant UML acro extension will generate the diagrams and create the image reference into the output document.
...
[source,bash]
----
java -cp $HOME/.m2/repository/com/javax0/jamal/jamal-engine/1.9.1-SNAPSHOT/jamal-engine-1.9.1-SNAPSHOT.jar:$HOME/.m2/repository/com/javax0/jamal/jamal-api/1.9.1-SNAPSHOT/jamal-api-1.9.1-SNAPSHOT.jar:$HOME/.m2/repository/com/javax0/jamal/jamal-tools/1.9.1-SNAPSHOT/jamal-tools-1.9.1-SNAPSHOT.jar:$HOME/.m2/repository/com/javax0/jamal/jamal-core/1.9.1-SNAPSHOT/jamal-core-1.9.1-SNAPSHOT.jar:$HOME/.m2/repository/com/javax0/jamal/jamal-cmd/1.9.1-SNAPSHOT/jamal-cmd-1.9.1-SNAPSHOT.jar: javax0.jamal.cmd.JamalMain options
----
...
Consider the use `{@options lenient}` along with the definition of the `default` macro.
...
It should only be eliminated at the very end of the processing on the top level, which can be achieved using the <<unescape,`unescape`>> macro.

[[unescape]]
==== xxii. `unescape`

The macro `unescape` will liberate the content of the `escape` macros, which were locked in eternally using the `escape*` format.

There are two different ways to invoke the macro.
One encloses the content to be liberated.
The other one has no content.
In this second case the macro schedules a closer object that does the liberation after the whole processing was done.
...
Note that in the first example the `unescape` macro was used with the `#` prefix, so that the several `{a...}` macros are evaluated before the macro `unescape` was executed.
If we look at the first example using the `@` prefix, as
...
The macro `unescape` works very simple.
It simply evaluates the input of the macro, or the whole result in case it works deferred setting a flag that tells `escape` that this time it should ignore the `\*` character.
If the `unescape` in the example above is using the `@` character then the content is evaluated with the flag set and it becomes equivalent to the
...
which is also erroneous, because `qqq` is not defined.
...
[source]
----
The current version 1.9.1-SNAPSHOT is older than the required version. It has to be newer.
----
...
To be consistent the macros `include`, `eval` also use the `[` and the `]` characters.
...
For example, for the environment variable `CHECK_STATE`, the system property is `check.state`.
...
* `JAMAL_CHECKSTATE` can switch off macro statefulness checking during macro registration.
...
You can switch off the functionality if this environment variable is set to `false`.
It may be needed if you want to use an older, prior 1.8.0 library or a library that does not follow this rule.
You can also set `System.setProperty(Macro.JAMAL_CHECKSTATE_SYS,"false");` programmatically.
It is used in two tests in the 1.8.0 release of Jamal, which uses the 1.7.9 version of the snippet macros to generate the documentation.
