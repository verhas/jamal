...
Usage: <main class> [-fhvx] [--dry-dry-run] [--dry-run] [-c=<macroClose>]
                    [-d=<depth>] [-e=<exclude>] [-i=<include>] [-o=<macroOpen>]
                    [-s=<sourceDirectory>] [-t=<targetDirectory>]
                    [-r=<transform> [<transform>]]... <inputFile> <outputFile>
      <inputFile>
      <outputFile>
  ...
  -e, --exclude=<exclude>    file name regex pattern to exclude from the
                               processing
  -f, --file                 convert a single file, specify nput and output
  -h, --help                 help
...
[NOTE]
====
Using the `#` character in front of a built-in macro instead of `@` will make the content evaluated before the macro.
...
The reason for this is precisely the situation we describe in this example below.
It isn't easy to read the closing strings when there are more than one.
...
The macros `comment` and `block` can be used the same way.
...
If you need the whitespace (e.g.: newline) in the output then you can put those int the `ident` macro.
