...
You can place these options into the first line of the document into a comment.
...
You can place any of the currently available options listed in the above example separated by spaces in any order.
...
By default, the extension saves the preprocessed document in the `.adoc` file.
...
It may be needed on some older machines, or in case of big documents to improve performance.

==== log

You may be interested how many times and when the output file was regenerated.
...
This will instruct the extension to append a line like
...
to the log file.
The name of the log file is the same as the output file with the extension `.log` appended.
For example, if you edit the file `README.adoc.jam` then the output file will be `README.adoc` and the log file will be `README.adoc.log`.

This log file can grow easily and become fat.
