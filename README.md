...
If you do not then read the documentation of the Jamal Maven plugin at 
...
1. When we define the macros `x` and `y` inside the `comment` macro it happens in a local scope of the `comment` macro. 
   ...
   When we defined the macro `y` it also starts with `:` and so it gets into the global scope.
...
The explicit specification that the content is a Jamal source may be necessary when the content of the macro to be evaluated starts with the `/` character.
...
`include` reads a file similarly to `import` but it starts a new scope for the processing of the included file.
...
If this is missing the default, `JavaScript` is assumed.
...
If we redefine `b` the value of `c` will still remain `1492` assuming `a` is still `14`.
...
Leading and trailing spaces of the opening and closing strings will be trimmed off. That way
...
After that the `<<@sep>>` restores the opening and closing strings to the inherited one and with these, it defines `a1` and `a2` and exports them.
...
* `JAMAL_STACK_LIMIT` sets the recursive call depth in macro evaluation.
  ...
  * The same time most Java implementations can handle this dept.
  ...
  This limit may be too much in your envoriment.
  Jamal may still throw StackOverflowError handling your.
