...
[source]
----
{@maven:load com.javax0.jamal:jamal-io:2.0.2-SNAPSHOT}
----

before using any macro from this package from the first time.
Also note, that using the `maven:load` macro requires that you configure the `jamal-io` package as safe in the `~/.jamal/setup.properties` or `~/.jamal/setup.xml` file.
...
For example, if you want to execute the Graphviz program, you can configure it in the `~/.jamal/setup.properties` file as:
...
When you build your code on your local system Graphiz is available, configured in your `~/.jamal/setup.properties` and works.
