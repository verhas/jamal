...
JUNIT dependency upped to 5.8.1

Handling of `lenient` option has changed. From now on `lenient` has to be a global option.
...
Options are not stored in option stores any more.
Options are simple `Identified` objects stored along with the user defined macros.
This also means that options can individually be exported, and the whole options store cannot be exported any more in one.
