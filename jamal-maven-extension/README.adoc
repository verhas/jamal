...
This call assumes that the file is already there, and at this point, the extension is supposed to find it.
...
[source]
----
mvn -Djamalize=true com.javax0.jamal:jamal-maven-plugin:1.11.1-SNAPSHOT:jamal
----
...
The Jamal project itself has the following `extensions.jam` file in the project's root directory:

[source]
----
include::../.mvn/extensions.jam[]
----

This will always use the latest, usually development version of the extension and keeps the `extensions.xml` file up to date.
When a new release is available it will be used after the first build.
