...
will instruct jamal to include the file in verbatim mode and only the lines from 1 to 13.

The Java implementation declares the names and types of the parameters and the helper tool automatically parses the input.
The Java code of the macro gets the values for the parameters parsed and converted.

Parameters can be integer numbers, strings, boolean values, regular expression patterns.
...
The macros have opening and closing strings, macro parameter parsing uses single characters.
The actual character is specified by the Java implementation of the macro.
...
The `snip:collect` macro collects snippets from files, and it does not convert anything from its input.
...
When macros needs parameters using the support classes, they look at different sources for the values.
