 ...
 * macro register {@code push()} method. For example the macro include calls {@link MacroRegister#push(Marker)} before
 * starting to process the content of a file and calls {@link MacroRegister#pop(Marker)} when it finished.
 * <p>
 * Macro level gets back to the previous state when the {@link MacroRegister#pop(Marker)} method is invoked. This happens
