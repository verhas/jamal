...
/**
 * This is a marker interface that signals that a class implementing the {@link Macro} interface is a special macro
 * that needs to be evaluated in the scope that was starting inside the macro. It means that the code of the macro will
 * see all user defined macros that are defined inside the macro use.<p>
 *
 ...
 * then it can be defined inside the {@code snippet} macro, for example:<p>
 *
 * <pre>{@code
 ...
 * macro. This is because the class {@code javax0.jamal.extensions.Snippet.evaluate()} implements this interface.<p>
 *
 * Macros that want to register non-global user defined macros must not implement this interface. If they do so the
 * defined macro will be registered into the scope that exists inside the macro itself and is
 *
 */
