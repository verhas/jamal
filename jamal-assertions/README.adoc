...
This macro with the `!` evaluates its content and then results the error message in case there was an error.
...
In order to use the macros the library has to be on the classpath.
...
The name of the macro is followed by the options , if there is any, between `(` and `)` characters.
The rest of the macro are the parameters.
...
For an example, let us have a look at the sample we have already visited in the previous section.
...
The assertion macro `assert:startsWith` has two mandatory parameters and the optional message as third parameter.
...
All the assertions interpret two options.
These are
...
The first option `trim` tells the assertion that the parameters should be stripped before the assertion is evaluated.
This will remove all white space from the start and the end of the parameters.
The second option reverses the outcome of the assertion.
...
If you want to check that the argument is empty OR blank then you should use the `trim` option.
...
If this macro is evaluated it will always result error.
The use of this assertion makes sense in part of the code that is conditionally evaluated, like a part of an `if` macro.
Note that this macro also interprets the `trim` option, though it has no parameters that could be trimmed.
The option `trim` can be used, but has no effect.
The option `not` is also interpreted.
If the option `not` is used, then the macro will result an empty string and will not error.
