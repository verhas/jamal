...
line before the one defining the constant, and add a regular expression with one capturing group.
...
If the regular expression is not found in the first line of the snippet, or there is no capturing group, then the line itself will result from the macro.
