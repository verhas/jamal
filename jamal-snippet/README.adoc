...
If you start the snippet right after the ``=` character then the spaces between the first non-space character and the `=` sign will be skipped.
For example:
...
When you `snip:define` a snippet you can either specify the file name and the line number using parops, or you can name an already existing snippet to inherit the location from.
This is useful when you create a new snippet replacing some part of an existing one.
The file name and the line number is used
...
{@snip:define mySnippetB=abra kadabra badir kebi}
...
The call to this macro deletes all collected snippets from memory.
...
[source]
----
../jamal-snippet/README.adoc.jam:1379:30
----
