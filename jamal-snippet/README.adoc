...
When the placeholder `$time` is used the macro will replace it with the last modified time of the file.
The time will be formatted as `yyyy-MM-dd HH:mm:ss` by default.
...
When the placeholder `$time` is used the macro will replace it with the last modified time of the directory.
The time will be formatted as `yyyy-MM-dd HH:mm:ss` by default.
This format can also be changed by defining the macro/option `dateFormat`.
