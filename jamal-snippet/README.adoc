...
That way, the document remains up-to-date, and you will not forget to update it.

To use the macro, you should first select some part of the code, which is documented to become a snippet.
There is no particular way for the purpose.
You have to do it the same way as you would do for any other snippet.
You just enclose the text between 
...
The hashcode can also be specified in the macro option `hash` before the snippet's name.

[source,text]
----
{@snip:check (hash=2a4ddeab580ad1fe8c95a) specialCodeToCheck}
----
...
It means that the snipped has changed, and the documentation has to follow the change.
...
You may notice that the example above does not contain that many.
...
2. Insert the `{@snip:check (hash=) NAME}` macro into the parts of the documentation where the part is documented.
3. Run the macro processing.
It will eventually result in an error.
The error message will include the hash code.
Copy at least six of the last characters to the macro, like `{@snip:check (hash=af6ed3) NAME}`
