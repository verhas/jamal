...
The default behaviour is that the lines will be any long and the paragraphs will be separated by double new-lines.
...
In some cases the limiting cannot be done.
When the width is too small, but still positive there may be some word longer than the given width. 
...
The `width` parameter can be set as a macro option as well as a macro, for example
...
will have again the same result:
...
This may be used to override a globally set `width` macro.

[[listDir]]
==== xvi. `listDir`

This macro list the files in a directory and then returns the list of the formatted files.
The format of the macro is:
...
* `maxDepth` the maximum dept of recursion into subdirectories.
Specify `1` in case you do not want to recurse into subdirectories.

* `followSymlinks` to follow symbolic links

* `countOnly` return the number of the files as a string instead of the list of the file names.
...
The macro `for` is used with the `#` character, so the macro `listDir` is evaluated before the `for` starts to execute.
The listing of the files is recursive and is unlimited by default.
The recursion can be limited by defining the parameter `maxDepth`.
...
then the recursive collection process for collection the files will follow symlinks.

The separator character between the formatted items is a comma by default.
It can be modified setting the option `separator` or its alias `sep`.
...
The macro `xmlFormat` interprets the input as an XML document if there is any, and it results the formatted document.
...
<groupId>com.javax0.jamal</groupId><artifactId>jamal-snippet</artifactId><version>1.7.10-SNAPSHOT</version>
</project>
}
----
...
That way the string can be used inside a string.
This macro is useful when the output is used in some programming language source.
Example:
...
If the option `ignoreCase` is used then the comparison is done ignoring the character casing.
