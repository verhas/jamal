...
[source]
----
/Users/verhasp/github/jamal/jamal-snippet/README.adoc.jam:1352:30
----
...
- FilesMacro.java: 8,220 bytes
- Collect.java: 23,699 bytes
...
The non-words and words, which are delimited by special characters seen below,() are not decorated.
...
When a word is split into multiple parts, the first part will be decorated using the first decorator, the second part will be decorated using the second decorator, and so on.
...
In this case, the past part of the word will ot be decorated.
...
The first character is either '-' or '+' signalling if common words should be decorated or not.
