...
The file's name must be specified by the parameter `output`.
...
The timestamp is present, so it is easier to use the value by programs.
At the same time, it is more human-friendly to have a date value.
...
When this XML has loaded, this file will be stored so that the snippet will look like one loaded from this file.

* `line` is the line number in the file where the snippet starts.
When this XML is loaded, this line number will be stored so that the snippet will look like one loaded from this line from the specified file.

* `column` is the column where the snippet was loaded from.
This value is usually `1` since snippets are multi-lined strings and do not contain fractional lines.
When this XML is loaded, the snippet will look like one loaded from this column from the specified line.
...
In that case, the snippet is loading terminates, and no more snippets are loaded from the XML file.
...
The format of th XML file has to be the same as the one used by the <<snipSave,`snip:save`>> macro.
It has to use the same name space, and tags.
The content of snippets must be in CDATA section(s).

The `ts`, and
`dateTime` attributes of the tag
`snippets` may be missing and are ignored during reading.
The attributes of the `snippet` tags are mandatory except the
`hash` attribute.
If the `hash` attribute is present its value will be checked against the content of the snippet.
This guarantees the snippet integrity.
If the tag is missing the check is skipped.

Any tag can have any extra attributes.
If you want to change something in the XML file and edit some snippet code temporarily you can rename the tag to `_hash` for example.
...
The snippet `collect_options` is defined in the file `Collect.java` at the line 42.
...
It can kill/keep lines, skip, replace, trim lines, it can reflow the lines, and it can number the lines.
...
These are `skip`, `kill` and `replace`.
...
You cannot, for example, specify `width` without adding `reflow` to the actions as it is not evident what the meaning of `width` is without `reflow`.

You will list the action in the `actions` parameter if you want to use it in a different place/order than the one it would get addedd implicitly.
...
The meaning and the interpreation of the parameters is the same as for the underlying macros and documented there.
...
- SnipSave.java: 4,309 bytes
- Case.java: 2,273 bytes
- NumberLines.java: 2,763 bytes
...
- BlockConverter.java: 1,715 bytes
...
<groupId>com.javax0.jamal</groupId><artifactId>jamal-snippet</artifactId><version>1.10.5-SNAPSHOT</version>
</project>
}
----
