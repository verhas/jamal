...
If the input DOCX file contains this macro the output DOCX file will be read only, and optionally protected by a password.
The syntax of the macro is:

  {@docx:readOnly [password=password algorithm=algorithm]}

If a password is defined, then an algorithm should also and the other way around.
The algorithm is used to encrypt the password.
There is no default algorithm.
The algorithms should be specified by the ECMA standard name of the algorithm.
...
If there is no algorithm defined the user can simply switch off the read only protection without any password.
If there is a password the user has to enter the password to switch off the read only protection.
Note, however, that this protection can be circumvented easily using some other software.

If this macro is invoked during the conversion of a normal text document an error will happen.
It is also an error to invoke this macro more than once in the same document.
