...
<dependency>
    ...
    <artifactId>jamal-snake</artifactId>
...
[source]
----
{c: null, a: this is c.a, b: this is c.b}
...
The resolution process will be simple unless the option `yamlResolveClone` (alias `clone`) is used before the name of the macro between `(` and `)` characters.
